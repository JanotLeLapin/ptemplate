package fr.janotlelapin.ptemplate.core.listeners;

import fr.janotlelapin.ptemplate.core.events.*;
import org.bukkit.*;
...
public class ArmorListener implements Listener {
    ...
    public final void inventoryClick(final InventoryClickEvent e) {
        ...
        ArmorType newArmorType = ArmorType.matchType(
            shift ? e.getCurrentItem() : e.getCursor()
        );
        ...
        if (shift) {
            ...
            if (newArmorType != null) {
                ...
                if (
                    ...
                    ArmorEvent armorEquipEvent = new ArmorEvent(
                        (Player) e.getWhoClicked(),
                        ArmorEvent.EquipMethod.SHIFT_CLICK,
                        newArmorType,
            ...
            if (numberkey) {
                if (e.getClickedInventory().getType().equals(InventoryType.PLAYER)) { // Prevents shit in the 2by2 crafting
                    ...
                    if (!isAirOrNull(hotbarItem)) { // Equipping
                        ...
                        newArmorType =
                            ArmorType.matchType(
                                !isAirOrNull(e.getCurrentItem()) ? e.getCurrentItem() : e.getCursor()
                            );
                    }
                }
            } else {
            ...
            if (newArmorType != null && e.getRawSlot() == newArmorType.getSlot()) {
                ...
                if (e.getAction().equals(InventoryAction.HOTBAR_SWAP) || numberkey) method =
                    ArmorEvent.EquipMethod.HOTBAR_SWAP;
                ArmorEvent armorEquipEvent = new ArmorEvent(
                    (Player) e.getWhoClicked(),
                    method,
                    newArmorType,
    ...
    public void inventoryDrag(InventoryDragEvent event) {
        ...
        if (
            type != null && type.getSlot() == event.getRawSlots().stream().findFirst().orElse(0)
        ) {
            ArmorEvent armorEquipEvent = new ArmorEvent(
                (Player) event.getWhoClicked(),
                ArmorEvent.EquipMethod.DRAG,
                type,
                null,
                event.getOldCursor()
            );
        ...
        for (ItemStack i : p.getInventory().getArmorContents()) {
            if (!isAirOrNull(i)) {
                Bukkit
                    ...
                    .getPluginManager()
                    .callEvent(
                        new ArmorEvent(p, ArmorEvent.EquipMethod.DEATH, ArmorType.matchType(i), i, null)
                    );
            }
        }
    }
